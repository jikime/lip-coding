...
async def root():
    ...
    <html>
        ...
        <body>
            <h1>멘토-멘티 매칭 API</h1>
            <p><a href="/docs">Swagger UI로 이동</a></p>
...
async def login(user: UserLogin):
    ...
    cursor.execute("SELECT id, password_hash, name, role FROM users WHERE email = ?", (user.email,))
...
async def get_current_user(current_user_id: int = Depends(verify_token)):
    ...
    profile_image_url = f"/api/images/{user['role']}/{user['id']}"
...
async def get_profile_image(
    ...
    cursor.execute("SELECT profile_image FROM users WHERE id = ? AND role = ?", (user_id, role))
    user = cursor.fetchone()
    ...
    if user["profile_image"]:
        return {"url": f"data:image/jpeg;base64,{base64.b64encode(user['profile_image']).decode()}"}
    else:
        # 기본 이미지 반환
        if role == "mentor":
            return {"url": "https://placehold.co/500x500.jpg?text=MENTOR"}
...
async def update_profile(
    ...
    if "image" in profile_data and profile_data["image"]:
        try:
            ...
            cursor.execute("UPDATE users SET profile_image = ? WHERE id = ?", (image_data, current_user_id))
        except:
            pass
    ...
    if user["role"] == "mentor" and "skills" in profile_data:
        ...
        cursor.execute("UPDATE mentors SET skills = ? WHERE user_id = ?", (skills_str, current_user_id))
...
async def accept_request(
    ...
    # 상태 업데이트
    cursor.execute("UPDATE match_requests SET status = 'accepted' WHERE id = ?", (request_id,))
    conn.commit()
    conn.close()
...
async def reject_request(
    ...
    # 상태 업데이트
    cursor.execute("UPDATE match_requests SET status = 'rejected' WHERE id = ?", (request_id,))
    conn.commit()
    conn.close()
...
async def delete_request(
    ...
    # 상태 업데이트
    cursor.execute("UPDATE match_requests SET status = 'cancelled' WHERE id = ?", (request_id,))
    conn.commit()
    conn.close()
